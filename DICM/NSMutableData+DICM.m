...
- (id)initDICMDataGroup2WithDICMDictionary:(NSDictionary*)dictionary
{
    if (self = [super init])
    {
        ...
        if ([array00020002 count]==0) NSLog(@"WARN [NSMutableData+DICM] group2 missing 00020002UI MediaStorageSOPClassUID");
        else
        {            
            ...
            if ([array00020003 count]==0) NSLog(@"WARN [NSMutableData+DICM] group2 missing 00020003UI MediaStorageSOPInstanceUID");
            else
            {
                ...
                if ([array00020010 count]==0) NSLog(@"WARN [NSMutableData+DICM] group2 missing 00020010UI TransferSyntaxUID");
                else
                {
                    ...
                    if ([array00020012 count]==0) NSLog(@"WARN [NSMutableData+DICM] group2 missing 00020010UI ImplementationClassUID");
                    else
                    {
                        ...
                        return self;//NSLog(@"INFO [NSMutableData+DICM] group2 initialized");
                    }
                }
            }
        }
...
- (NSStringEncoding)appendDICMAttribute:(NSString*)tagPath withObject:(id)object bulkdataBaseURI:(NSURL*)bulkdataBaseURI stringEncoding:(NSStringEncoding)stringEncoding
{
    ...
    if ((tagPathLength<10) || ((tagPathLength%10)!=0)) NSLog(@"ERROR [DICMData] bad tagPath '%@'",tagPath);
    else
    {
...
#pragma mark LO LT PN SH ST                
 ...
 #pragma mark UT               
            case 0x5455:;//UT not multivalued
                ...
                else  NSLog(@"ERROR [DICMData] tagPath '%@' can´t contain multi values",tagPath);
                break;
...
#pragma mark FL FD SL SS UL US AT
            ...
            case 0x5441://AT
            {
                ...
                for (NSString *ATTagPath in object)
                {
                    ...
                    if ((ATTagPathLength<9) || ((ATTagPathLength%10)!=0)) NSLog(@"ERROR [DICMData] bad AT tag '%@',",ATTagPath);
                    else
                    {
...
#pragma mark KB KF KW KN (bulkdata URI to be fetched before incorporation into the DICOM file)
            ...
            case 0x4E4B://KN
            {
                ...
                else NSLog(@"ERROR [DICMData] bulkdataURL %@ is not reachable",object);
                break;
            }
...
#pragma mark QK QX QZ JX JZ KG GX GZ
            ...
            case 0x594B://KY
            {
                ...
                if (!fragmentBulkData) NSLog(@"ERROR [DICMData] bulkdataURL %@ is not reachable",object);
                else
                {
                    [self appendBytes:&ItemInitialTag length:4];                    
            ...
            default:
            {
                NSLog(@"WARN -> [DICMData] unknown %@ vr for tag %u",vr, (unsigned int)tagLe);
                break;
            }
        }
    }
