...
- (void) setDICMValue:(NSNumber*)value forKey:(NSString*)key
{
	...
	if (strcmp([value objCType], @encode(short)) == 0)
	{
		...
		else										NSLog(@"ERROR -> [NSMutableDictionary] mismatch: value short for %@", key);	
	}	
	else if (strcmp([value objCType], @encode(unsigned short)) == 0)
	{
		...
		else										NSLog(@"ERROR -> [NSMutableDictionary] mismatch: value unsigned short for %@", key);	
	}
	else if (strcmp([value objCType], @encode(long)) == 0)
	{
		...
		else										NSLog(@"ERROR -> [NSMutableDictionary] mismatch: value long for %@", key);	
	}
	else if (strcmp([value objCType], @encode(unsigned long)) == 0)
	{
		...
		else										NSLog(@"ERROR -> [NSMutableDictionary] mismatch: value unsigned long for %@", key);	
	}
	else if (strcmp([value objCType], @encode(float)) == 0)
	{
		...
		else										NSLog(@"ERROR -> [NSMutableDictionary] mismatch: value float for %@", key);	
	}
	else if (strcmp([value objCType], @encode(double)) == 0)
	{
		...
		else										NSLog(@"ERROR -> [NSMutableDictionary] mismatch: value double for %@", key);	
	}		
}
...
-(void)setCodeSequenceForKey:(NSString*)k
                        array:(NSArray*)array
{
#pragma mark TODO add translation
    ...
    if (arrayCount)
    {
        ...
        {
            NSDictionary *codeDict=array[i];
            NSString *conceptNameCodeSequenceItem = [self setDICMItemForKey:conceptNameCodeSequence index:i+1];
            [self setObject:[NSArray arrayWithObject:codeDict[@"code"]] forKey:[conceptNameCodeSequenceItem stringByAppendingString:@"00080100SH"]];
            [self setObject:[NSArray arrayWithObject:codeDict[@"scheme"]] forKey:[conceptNameCodeSequenceItem stringByAppendingString:@"00080102SH"]];
            [self setObject:[NSArray arrayWithObject:codeDict[@"meaning"]] forKey:[conceptNameCodeSequenceItem stringByAppendingString:@"00080104LO"]];
        }
