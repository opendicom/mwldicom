                                ...
                                reading:(NSString*)reading
{
	...
	[DICMC070201 setDICMString:referring	forKey:@"00080090PN"];//ReferringPhysicianName 2
	...
	[DICMC070201 setDICMString:description forKey:@"00081030LO"];//StudyDescription 3

    if ([codes count]) [DICMC070201 setCodeSequenceForKey:@"00081032SQ" array:codes];//00081032 Procedure Code Sequence
    
    
    //none = "*", becomes significant when > 1
    if (reading && (reading.length > 1))
        [DICMC070201 setDICMString:reading forKey:@"00081060PN"];//NameOfPhysiciansReadingStudy
...
+(NSDictionary*)DICMC070301ForModality1:(NSString*)modality seriesUID1:(NSString*)seriesUID seriesNumber2:(NSString*)seriesNumber laterality2c:(NSString*)laterality seriesDate3:(NSDate*)seriesDate seriesTime3:(NSDate*)seriesTime performingPhysiciansName3:(NSString*)performingPhysiciansName protocolName3:(NSString*)protocolName seriesDescription3:(NSString*)seriesDescription bodyPartExamined3:(NSString*)bodyPartExamined patientPosition2c:(NSString*)patientPosition smallestPixelValueInSeries3:(NSValue*)smallestPixelValueInSeries largestPixelValueInSeries3:(NSValue*)largestPixelValueInSeries
{
	...
	//1
	if (!modality)							{ NSLog(@"ERROR -> [NSDictionary] DICMC070301 modality 1 nil"); return nil;}
	if ([modality isEqualToString:@""])		{ NSLog(@"ERROR -> [NSDictionary] DICMC070301 modality 1 empty"); return nil;}
	[DICMC070301 setDICMString:modality	forKey:@"00080060CS"];
	
	//1
	if (!seriesUID)							{ NSLog(@"ERROR -> [NSDictionary] DICMC070301 seriesUID 1 nil"); return nil;}
	if ([seriesUID isEqualToString:@""])	{ NSLog(@"ERROR -> [NSDictionary] DICMC070301 seriesUID 1 empty"); return nil;}
	[DICMC070301 setDICMString:seriesUID forKey:@"0020000EUI"];
	
	//2
	if (!seriesNumber)						{ NSLog(@"ERROR -> [NSDictionary] DICMC070301 seriesNumber 2 nil"); return nil;}
	...
	//3
	if (seriesDate) [DICMC070301 setDICMString:[DICMTypes DAStringFromDate:seriesDate] forKey:@"00080021DA"];
	
	//3
	if (seriesTime)	[DICMC070301 setDICMString:[DICMTypes TMStringFromDate:seriesTime]	forKey:@"00080031TM"];
...
+(NSDictionary*)DICMC070501ForInstitution:(NSString*)institution
{
	//===================================
	...
	//3
	if (institution) [DICMC070501 setDICMString:institution	forKey:@"00080080LO"];//InstitutionName
...
+(NSDictionary*)DICMC080601ForConversionType1:(NSString*)conversionType
{
	...
	//1
	if (!conversionType)					{ NSLog(@"ERROR -> [NSDictionary] DICMC080601 Conversion Type 1 nil"); return nil;}
	...
	else { NSLog(@"ERROR -> [NSDictionary] DICMC070301 Conversion Type 1 not standard:%@",conversionType); return nil;}
...
#pragma mark TODO investigate why %0000d doesn't work anymore
			...
			if (spatialLocationsPreserved)
			{
				...
				else if ([spatialLocationsPreserved isEqualToString:@"REORIENTED_ONLY"])
				{
					...
					else NSLog(@"ERROR -> [DICMC070601] Spatial Location Preserved requires (0020,0020) Patient Orientation, which is missing");
				}
				else NSLog(@"ERROR -> [DICMC070601] Spatial Location Preserved mismatch [YES|NO|REORIENTED_ONLY]:%@",spatialLocationsPreserved);
			}		
	...
	if (burnedInAnnotation) 
	{
		...
		else NSLog(@"ERROR -> [DICMC070601] Burned In Annotation mismatch [YES|NO]:%@",burnedInAnnotation);
	}
...
+(NSDictionary*)DICMC170602ForUIDUIDUIDStrings1:(NSArray*)UIDUIDUIDStrings
{
	...
	if (!UIDUIDUIDStrings) NSLog(@"ERROR -> [Dictionary_DICM] DICMC170602ForUIDUIDUIDStrings1 array nil");
	else
	{
		NSInteger UIDUIDUIDcount = [UIDUIDUIDStrings count];
		//if (UIDUIDUIDcount == 0) NSLog(@"ERROR -> [Dictionary_DICOM] DICMC170602ForUIDUIDUIDStrings1 array empty");
		//else 
		//{
...
+(NSDictionary*)DICMC240200EncapsulatedPDFWithDA:(NSString*)DA
                                              TM:(NSString*)TM
                                           title:(NSString*)title
                                            data:(NSData*)data
{
    NSMutableDictionary *DICMC240200 = [NSMutableDictionary dictionaryWithCapacity:10];
    ...
    [DICMC240200 setDICMString:@"application/pdf" forKey:@"00420012ST"];//00420010 DocumentTitle 2
...
+(NSDictionary*)DICMC240200EncapsulatedPDFWithDA:(NSString*)DA
                                              TM:(NSString*)TM
                                           title:(NSString*)title
                                          b64pdf:(NSString*)b64pdf
{
    NSData *pdf=[[NSData alloc]
                 initWithBase64EncodedString:b64pdf
    ...
    [DICMC240200 setDICMString:TM forKey:@"00400245TM"];//PerformedProcedureStepStartTime
    [DICMC240200 setDICMString:title forKey:@"00420010ST"];//00420010 DocumentTitle 2
    [DICMC240200 setDICMData:pdf forKey:@"00420011OB"];
    //00420010 DocumentTitle 2
    [DICMC240200 setDICMString:@"application/pdf" forKey:@"00420012ST"];//00420010 DocumentTitle 2
...
+(NSDictionary*)DICMC120100ForSOPClassUID1:(NSString*)instanceclass
                           SOPInstanceUID1:(NSString*)instanceUID
	...
	[DICMC120100 setDICMString:instanceclass	  forKey:@"00080016UI"];//SOPClassUID	1
	[DICMC120100 setDICMString:instanceUID forKey:@"00080018UI"];//SOPInstanceUID 1
